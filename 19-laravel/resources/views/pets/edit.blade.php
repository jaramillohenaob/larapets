        ...
        <div class="card text-white md:w-[720px] w-[320px] bg-black/20 p-4 mb-4 rounded">
            <form method="POST" action="{{ url('pets/'.$pet->id) }}" class="flex flex-col md:flex-row gap-4 mt-4" enctype="multipart/form-data">
                ...
                <div class="w-full md:w-[320px]">
                    ...
                    <textarea class="textarea bg-[#0009] outline-0 w-full" name="description" placeholder="Brief description about the pet..." rows="3">{{ old('description', $pet->description) }}</textarea>
                    @error('description')
                        <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                    @enderror
                    {{-- Active --}}
                    ...
                    <select name="adopted" class="select bg-[#0009] outline-0">
                        <option value="0" @if(old('adopted', $pet->adopted) == '0') selected @endif>Available</option>
                        <option value="1" @if(old('adopted', $pet->adopted) == '1') selected @endif>Adopted</option>
                    </select>
